...
def momentcube (hdu, v1, v2, header=None, 
    ...
    if moment not in range(3):
        raise CubeVisArgumentError('moment', "moment can only be one of 0,1,2")
    ...
    if returnrms and window is None:
        raise CubeVisArgumentError('rms', 'if returnrms is True, you need to specify a window to do the rms calculation in')
    if returnrms and moment != 0:
        raise CubeVisArgumentError('rms', 'For now, only moment=0 return rms image')
    ...
    T = numpy.nansum(data[:,:,vind], axis=2) #T is integ intensity image now
    if moment >= 1:
        C = numpy.nansum(data[:,:,vind]*velax[vind], axis=2)
        C = C/T  #centroid velocity definition
        if moment == 2:
            W = numpy.nansum(data[:,:,vind]*velax[vind]**2., axis=2)
            W = W/T - C**2.
