...
import numpy
import copy
from cubevis.idealpy.utils.congrid import congrid
...
def transpose_header(header, origin='vxy'):
    ...
    cards = header.cards
    keepattr = {}
    for attr in ('NAXIS', 'CTYPE', 'CRVAL', 'CDELT', 'CRPIX', 'CUNIT', 'CROTA'):
        keepattr[attr] = {}
        for i in range(1, 4):
            if '%s%d' % (attr,i) in header:
                keepattr[attr][i] = copy.copy(cards['%s%d' % (attr,i)])
    print(keepattr)
    if origin == 'vxy':
        for attr in ('NAXIS', 'CTYPE', 'CRVAL', 'CDELT', 'CRPIX', 'CUNIT', 'CROTA'):
            if '%s' % attr in keepattr and len(keepattr[attr]) > 0:
                #1->3
...
def transpose_cube(hdu, origin='xyv',
    ...
    if origin[2] == 'v':
        if header.get('CTYPE3') not in ('VELO-LSR', 'VELO_LSR', 'VELOCITY'):
            raise CubeVisArgumentError('header', 'Input Cube does not have Velocity in 3rd axis')
    else:
        if header.get('CTYPE1') not in ('VELO-LSR', 'VELO_LSR', 'VELOCITY'):
            raise CubeVisArgumentError('header', 'Input Cube does not have Velocity in 1st axis')
