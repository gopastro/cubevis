...
from cubevis.idealpy.radio.smooth_image import gauss_kern
...
def extract_spec(hdu, x0, y0, header=None, gauss_width=2):
    ...
    if x0 < 0 or x0 >= xsize:
        raise CubeVisArgumentError('x0', "x0=%s is out of bounds of x-limits: (0, %d)" % (x0, xsize))
    if y0 < 0 or y0 >= ysize:
        raise CubeVisArgumentError('y0', "y0=%s is out of bounds of y-limits: (0, %d)" % (y0, ysize))
    #print gk.shape
